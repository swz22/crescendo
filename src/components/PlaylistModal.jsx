...
const PlaylistModal = ({ playlist, initialMosaicImages, onClose }) => {
  ...
  // Use the passed mosaic images from the card
  useEffect(() => {
    if (initialMosaicImages && initialMosaicImages.length === 4) {
  ...
  return (
    <>
            ...
            {tracks && (
              <div className="space-y-1">
                {tracks.map((track, i) => (
                  ...
                  >
                    <span className="text-gray-500 text-sm w-8 text-center">{i + 1}</span>
      ...
      <FloatingMiniPlayer isVisible={isAnimating} />
    </>
  );
};
