...
const DetailsHeader = ({ artistId, artistData, songData }) => {
  ...
  const getArtistImage = () => {
    ...
    if (artistData?.artist?.images?.[0]?.url)
      return artistData.artist.images[0].url;
  ...
  const getSongTitle = () => {
    return songData?.title || songData?.attributes?.name || "Unknown Song";
  };
  ...
  const getGenre = () => {
    ...
    return songData?.genres?.primary || songData?.genre || "Hip-Hop/Rap";
  };
      ...
      <div className="absolute inset-0 flex items-center">
        ...
        <div className="ml-5">
          ...
          {!artistId && songData && (
            <Link
              to={`/artists/${
                songData?.artists?.[0]?.adamid || songData?.artists?.[0]?.id
              }`}
            >
          ...
          <p className="text-base text-gray-400 mt-2">{getGenre()}</p>
        </div>
      </div>
