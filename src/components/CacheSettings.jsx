...
const CacheSettings = () => {
  ...
  return (
    <>
      {/* Settings Button - now in top left */}
      <button
        onClick={() => setShowSettings(!showSettings)}
        className="relative bg-white/10 hover:bg-white/20 text-white p-2.5 rounded-full transition-all duration-200 backdrop-blur-sm group"
      ...
      {showSettings && (
        <div className="absolute top-16 left-0 z-50 bg-gradient-to-br from-purple-900/95 to-black/95 backdrop-blur-xl rounded-lg shadow-2xl border border-white/10 p-6 min-w-[300px] animate-slidedown">
          <h3 className="text-white font-bold text-lg mb-4">Cache Settings</h3>
          
          {/* Preview URL Cache Stats */}
          <div className="mb-4">
            <h4 className="text-white font-medium text-sm mb-2">Preview URL Cache</h4>
            {stats && (
          ...
          {/* Audio Cache Stats */}
          <div className="mb-4">
            <h4 className="text-white font-medium text-sm mb-2">Preloaded Audio</h4>
            {audioStats && (
              <div className="space-y-1 pl-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Cached Audio:</span>
                  <span className="text-white">{audioStats.cached}/{audioStats.maxCache}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Loading:</span>
                  <span className="text-white">{audioStats.loading} tracks</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Memory Usage:</span>
                  <span className="text-white">~{(audioStats.cached * 3).toFixed(0)} MB</span>
                </div>
              </div>
            )}
          </div>
